...
class Patient_resource_model extends CI_Model {
	...
	public function add($data)
	{
	 ...
	 public function getReportdetails(){
		$this->db->select('*');
		$today=date('y-m-d');
		$arr=array('apmt_date'=>$today,'status'=>0);
		$this->db->where($arr);
		return $this->db->get('tbl_doctorprescription')->result();
		 }
	 ...
	 public function getQuery($id){
		 $gid=dycrypt($id);
		 $this->db->select()->from('tbl_doctorprescription');
		 $this->db->where('id',$gid);
		 return $this->db->get()->row_array();
		 }
	 ...
	 public function addReportdetails($newreg){
		 $today=date('y-m-d');
		 $new=encrypt($newreg);
		 ...
		 //$today=date('y-m-d');
		 $data=array('opno'=>$opno,'value'=>$new,'apmt_date'=>$today,'doctor'=>$doctor,'staff'=>$staff,'crea_date'=>$today,'mod_date'=>$today);
		 //$data=array('opno'=>$opno,'value'=>$new,'apmt_date'=>$today,'doctor'=>$doctor);
		 $this->db->insert('tbl_patientreport',$data);
		 }
