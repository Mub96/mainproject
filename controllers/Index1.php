...
swal('Your Appointment has been successfully booked');
  ...
  /*function create_pdf($insert_id,$email)
 {
	...
	public function general_appointment()
	{
		...
		$get_appoint=array($firstname,$lastname,$dob,$addr,$city,$state,$zipcode,$phn,$email,$c_email,$reach,$type_of_appointment,$reason,$hear_about_us,$list,$preferred_day,$preferred_time);
...
try {
  ...
  $mail->AddReplyTo('web.zshoper@gmail.com');
  $mail->AddAddress($email);
  $mail->SetFrom('web.zshoper@gmail.com');
  $mail->AddReplyTo('web.zshoper@gmail.com');
...
try {
  ...
  $mail->Host       = "smtp.gmail.com"; // sets the SMTP server
  $mail->Port       = 465;                    // set the SMTP port for the GMAIL server
  $mail->Username   = "help@cypressgastro.com"; // SMTP account username
  $mail->Password   = "";        // SMTP account password
